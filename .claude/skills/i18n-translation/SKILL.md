...
| 场景 | 位置 | 示例 |
|------|------|------|
| 页面专用文本（Modal、表单、按钮） | `pages.{pageName}.xxx` | `pages.aiProviderManager.modal.name.label` |
| 表格列标题 | `components.columns.{entityName}.xxx` | `components.columns.aiModel.name` |
...
| 系统设置 | `pages.systemSettingsManager.{keys\|groups\|tabs\|enums}` | `pages.systemSettingsManager.keys.mail.smtp.host` |
| 租户设置 | `pages.tenantSettingsManager.{keys\|groups\|tabs\|enums}` | `pages.tenantSettingsManager.keys.xxx` |
...
| `t('enums.status.enabled')` (页面特定的) | `t('pages.xxx.modal.enabled.enabled')` 或 `t('enums.xxx.enabled')` | 区分全局枚举和页面特定文本 |
...
3. 修改的文件列表（zh-CN.ts, en-US.ts, 以及其他相关文件）
4. TypeScript 类型检查结果
5. 如果是枚举或设置项，说明是否已完成注册/映射

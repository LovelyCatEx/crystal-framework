...
export function useTenantMemberTableColumns(): EntityTableColumns<TenantMemberVO> {
    const { t } = useTranslation();

    return [
        {
            title: t('components.columns.tenantMember.recordInfo'),
            dataIndex: "id",
            key: "id",
            render: function (_: unknown, row: TenantMemberVO): React.ReactNode | JSX.Element {
                return <Space orientation='vertical' size={0}>
                    ...
                    <CopyableToolTip title={row.tenantId}>
                        <Tag color="orange" className="m-0 text-[10px] leading-4 h-4 px-1 rounded">{t('components.columns.tenantMember.tenantId')}: {row.tenantId}</Tag>
                    </CopyableToolTip>
                </Space>
            }
        },
        {
            title: t('components.columns.tenantMember.userInfo'),
            dataIndex: "user",
            key: "user",
            render: function (_: unknown, row: TenantMemberVO): React.ReactNode | JSX.Element {
                const user = row.user;
                    ...
                    <Popover content={<UserCard userId={user.id} />} placement="right" trigger="hover">
                        <Space orientation='horizontal' size={8} className="cursor-pointer">
                            ...
                            <Space orientation='vertical' size={0}>
                                ...
                                <Tag color="purple" className="m-0 text-[10px] leading-4 h-4 px-1 rounded">{t('components.columns.tenantMember.userId')}: {user.id}</Tag>
                            </Space>
                        </Space>
                    </Popover>
                );
        ...
        {
            title: t('components.columns.tenantMember.email'),
            dataIndex: "email",
            key: "email",
            render: function (_: unknown, row: TenantMemberVO): React.ReactNode | JSX.Element {
                return <Space orientation='vertical' size={0}>
        ...
        {
            title: t('components.columns.tenantMember.status'),
            dataIndex: "status",
            key: "status",
            render: function (_: unknown, row: TenantMemberVO): React.ReactNode | JSX.Element {

...
export function useTenantMessageChannelTableColumns(): EntityTableColumns<TenantMessageChannel> {
    const {t} = useTranslation();

    return [
        {
            title: t('components.columns.tenantMessageChannel.recordInfo'),
            dataIndex: "id",
            key: "id",
            render: function (_: unknown, row: TenantMessageChannel): React.ReactNode | JSX.Element {
                return <Space orientation='vertical' size={0}>
                    <CopyableToolTip title={row.id}>
                        <Tag color="blue" className="m-0 text-[10px] leading-4 h-4 px-1 rounded">ID: {row.id}</Tag>
                    </CopyableToolTip>
                    <CopyableToolTip title={row.tenantId}>
                        <Tag color="orange" className="m-0 text-[10px] leading-4 h-4 px-1 rounded">{t('components.columns.tenantMessageChannel.tenantId')}: {row.tenantId}</Tag>
                    </CopyableToolTip>
                </Space>
            }
        },
        {
            title: t('components.columns.tenantMessageChannel.name'),
            dataIndex: "name",
            key: "name",
            render: function (_: unknown, row: TenantMessageChannel): React.ReactNode | JSX.Element {
        ...
        {
            title: t('components.columns.tenantMessageChannel.channelType'),
        ...
        {
            title: t('components.columns.tenantMessageChannel.config'),
            dataIndex: "config",
            key: "config",
            render: function (_: unknown, row: TenantMessageChannel): React.ReactNode | JSX.Element {

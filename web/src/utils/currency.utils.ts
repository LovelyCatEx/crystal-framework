...
/**
 * Assembles the final display string from already-split integer/decimal digit strings, applying the
 * currency's thousands separator, decimal separator and symbol position.
 */
function assembleCurrency(
    intDigits: string,
    decDigits: string,
    precision: number,
    sign: string,
    ...
    const { symbol, symbolPosition, decimalSeparator, thousandsSeparator } = currency;
...
export function formatCurrencyUnits(units: string, currency: CurrencyEntity): string {
    ...
    return assembleCurrency(intDigits, decDigits, precision, sign, currency);
}

/**
 * Formats a raw decimal amount (e.g. a per-million price or a charged cost, already in the
 * currency's display scale) into a human-readable amount, rounding to the currency's precision and
 * applying its symbol position, decimal separator and thousands separator.
 */
export function formatCurrencyAmount(amount: number | string, currency: CurrencyEntity): string {
    const { precision } = currency;
    const value = Number(amount ?? 0);
    const abs = Number.isFinite(value) ? Math.abs(value) : 0;
    const sign = value < 0 ? '-' : '';

    const [intDigits, decDigits = ''] = abs.toFixed(precision).split('.');

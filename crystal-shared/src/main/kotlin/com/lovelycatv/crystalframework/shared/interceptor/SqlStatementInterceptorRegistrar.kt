package com.lovelycatv.crystalframework.shared.interceptor
...
/**
 * Installs the [com.lovelycatv.crystalframework.shared.config.SqlStatementInterceptor] chain into [CrystalFrameworkSQLModifier] once every
 * singleton exists.
 *
 * The wiring must not sit in [com.lovelycatv.crystalframework.shared.config.database.R2dbcSQLInterceptorConfig]: its `databaseClient` bean is a foundational
 * R2DBC bean pulled in before component scanning finishes, so a constructor-injected interceptor list
 * there resolves to empty. [org.springframework.beans.factory.SmartInitializingSingleton.afterSingletonsInstantiated] runs at the end

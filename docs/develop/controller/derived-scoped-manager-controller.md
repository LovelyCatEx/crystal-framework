...
与 [StandardScopedManagerController](./scoped-manager-controller) 一致——16 个权限的 `PermissionMatrix`（4 层 × 4 操作）。区别在 scope 来源：Scoped 从实体的 scope 列读，DerivedScoped 从三个抽象方法返回。

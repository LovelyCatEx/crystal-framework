...
后端首次启动且系统尚未初始化时，会由一个实例在日志中输出一次性系统初始化令牌。该令牌存储在 Redis 中，普通重启不会改变，其他实例也不会重复打印。

打开浏览器访问前端开发服务器地址，进入系统初始化页面，填写日志中的令牌、Root 账号和邮件配置。初始化成功后令牌立即失效，随后使用新建的 Root 账号登录管理后台。

...
/**
 * Registers the framework's built-in [EconomyReferenceType]s. `HIGHEST_PRECEDENCE` so third-party
 * configurers observe the built-ins already present and can fail-fast on typeId / key collisions.
 */
@Component
@Order(Ordered.HIGHEST_PRECEDENCE)
class BuiltinEconomyReferenceTypeConfigurer : EconomyReferenceTypeConfigurer {
    override fun configure(registry: EconomyReferenceTypeRegistry) {
        registry.registers(EconomyReferenceType.entries)
    }
}

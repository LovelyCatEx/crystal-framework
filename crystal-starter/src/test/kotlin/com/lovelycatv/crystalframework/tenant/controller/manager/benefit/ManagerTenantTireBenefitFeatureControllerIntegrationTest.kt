...
/**
 * Integration test for [ManagerTenantTireBenefitFeatureController] — Standard SYSTEM-only.
 */
class ManagerTenantTireBenefitFeatureControllerIntegrationTest(
    @Autowired private val managerTenantTireBenefitFeatureController: ManagerTenantTireBenefitFeatureController,
    ...
    private val matrix: PermissionMatrix = PermissionMatrix.systemOnly(
        ...
        superCreate = SystemPermission.ACTION_TENANT_TIRE_BENEFIT_FEATURE_CREATE,
        superRead = SystemPermission.ACTION_TENANT_TIRE_BENEFIT_FEATURE_READ,
        superUpdate = SystemPermission.ACTION_TENANT_TIRE_BENEFIT_FEATURE_UPDATE,
        superDelete = SystemPermission.ACTION_TENANT_TIRE_BENEFIT_FEATURE_DELETE,
    )

    private fun readDto() = ManagerReadTenantTireBenefitFeatureDTO(page = 1, pageSize = 20)
    ...
    fun readEndpointAuthorizesOnlySuperLayer(layer: PermissionMatrix.Layer) {
        withTransactionalRollback("tenant-tire-benefit-feature-read-layer-$layer") {
            val user = setupUserForLayer(layer, ScopedOperation.READ)
            var caught: Throwable? = null
            withAuthenticatedUser(user) {
                caught = runCatching { managerTenantTireBenefitFeatureController.read(user.authentication, readDto()) }.exceptionOrNull()
            }
            when (layer) {
    ...
    fun readAllEndpointAuthorizesOnlySuperLayer(layer: PermissionMatrix.Layer) {
        withTransactionalRollback("tenant-tire-benefit-feature-readAll-layer-$layer") {
            val user = setupUserForLayer(layer, ScopedOperation.READ)
            var caught: Throwable? = null
            withAuthenticatedUser(user) {
                caught = runCatching { managerTenantTireBenefitFeatureController.readAll(user.authentication) }.exceptionOrNull()
            }
            when (layer) {
    ...
    fun readEndpointDeniesUserWithoutMatrixAuthorities() {
        withTransactionalRollback("tenant-tire-benefit-feature-read-denied-no-auth") {

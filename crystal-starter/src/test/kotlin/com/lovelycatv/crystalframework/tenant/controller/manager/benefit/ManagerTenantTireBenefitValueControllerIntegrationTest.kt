...
/**
 * Integration test for [ManagerTenantTireBenefitValueController] — Standard SYSTEM-only.
 */
class ManagerTenantTireBenefitValueControllerIntegrationTest(
    @Autowired private val managerTenantTireBenefitValueController: ManagerTenantTireBenefitValueController,
    ...
    private fun readDto() = ManagerReadTenantTireBenefitValueDTO(page = 1, pageSize = 20)
    ...
    fun readEndpointAuthorizesOnlySystemLayer(layer: PermissionMatrix.Layer) {
        withTransactionalRollback("tenant-tire-benefit-value-read-layer-$layer") {
            val user = setupUserForLayer(layer, ScopedOperation.READ)
            var caught: Throwable? = null
            withAuthenticatedUser(user) {
                caught = runCatching { managerTenantTireBenefitValueController.read(user.authentication, readDto()) }.exceptionOrNull()
            }
            when (layer) {
    ...
    fun readAllEndpointAuthorizesOnlySystemLayer(layer: PermissionMatrix.Layer) {
        withTransactionalRollback("tenant-tire-benefit-value-readAll-layer-$layer") {
            val user = setupUserForLayer(layer, ScopedOperation.READ)
            var caught: Throwable? = null
            withAuthenticatedUser(user) {
                caught = runCatching { managerTenantTireBenefitValueController.readAll(user.authentication) }.exceptionOrNull()
            }
            when (layer) {
    ...
    fun readEndpointDeniesUserWithoutMatrixAuthorities() {
        withTransactionalRollback("tenant-tire-benefit-value-read-denied-no-auth") {
